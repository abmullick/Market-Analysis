...
def _verify_report_heading(page: Any, market_type: str, hidden_date: str) -> None:
    ...
    logger.warning(
        ...
        hidden_date,
        expected,
        _normalize_heading_text(region_text)[:200],
    )
            ...
            try:
                ...
                try:
                    ...
                    page.locator("#filter_select").select_option("A")
                     ...
                     # Diagnostic: verify date values immediately before search.
                    before_click_visible = page.locator(
                        "#idtradedate"
                    ).input_value()
                    before_click_hidden = page.locator(
                        "#idhdndate"
                    ).evaluate("(el) => el.value || ''")

                    logger.warning(
                        "CDSL DATE BEFORE CLICK: market_type=%s, "
                        "visible=%r, hidden=%r, expected=%r",
                        market_type,
                        before_click_visible,
                        before_click_hidden,
                        hidden_date,
                    )
                    
                    # Reassert the requested date after market/filter
                    # selection, because page JavaScript may reset it.
                    page.locator("#idtradedate").fill(display_date)
                    page.locator("#idhdndate").evaluate(
                        "(el, value) => {"
                        " el.value = value;"
                    ...
                    # Diagnostic: inspect the request generated by Search.
                    def log_search_request(request):
                        if request.method != "GET" or request.post_data:
                            logger.warning(
                                "CDSL SEARCH REQUEST: method=%s, url=%s, "
                                "post_data=%r",
                                request.method,
                                request.url,
                                request.post_data,
                    ...
                    page.locator("form#ctl00").evaluate(
                        """(form, values) => {
                            form.addEventListener("submit", () => {
                                const visible =
                                    document.querySelector("#idtradedate");
                                const hidden =
                                    document.querySelector("#idhdndate");

                                if (visible) {
                                    visible.value = values.display_date;
                                }

                                if (hidden) {
                                    hidden.value = values.hidden_date;
                                }
                            }, true);
                        }""",
                        {
