// Portfolio Builder — AI Insights DOM renderers.
//
// Mirrors the Mutual Fund AI Insights response conventions
// (frontend/js/features/mutual-fund-analysis/ai-response.js) including the
// `fund-ai-insights-*` class names so the polished card, loading and error
// UI share the same theme. Only the disclosure text is portfolio-specific.
...
export function renderInsightResponse(container, response) {
    ...
    appendListSection(documentRef, container, "Strengths & Key Points", response.key_points);
    ...
    disclosure.textContent = "AI interpretation of the portfolio's deterministic analysis — not financial advice.";
