...
def test_fetcher_get_schemes_by_category(fetcher):
    ...
    with patch.object(fetcher.mfapi, "fetch_scheme", new_callable=AsyncMock, return_value=mock_raw):
        equity = asyncio.run(fetcher.get_schemes_by_category("Equity"))
        assert len(equity) == 1
        assert equity[0].scheme_name == "Large Cap Fund"

        large_cap = asyncio.run(fetcher.get_schemes_by_category("Large Cap"))
